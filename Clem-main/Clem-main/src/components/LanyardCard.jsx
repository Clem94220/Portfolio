...
function getErrorCopy(error) {
    if (error?.code === 'user_not_monitored') {
        return {
            title: 'Presence non configuree',
            description: 'Le compte Discord n est plus suivi par Lanyard. Le reste du site fonctionne normalement.',
        };
    }

    return {
        title: 'Presence indisponible',
        description: 'La connexion Discord est temporairement indisponible. Reconnexion automatique...',
    };
}
...
export default function LanyardCard({ data, loading, error }) {
    ...
    if (error || !data || !data.discord_user?.id) {
        const copy = getErrorCopy(error);

        return (
            <div className="crystal-card overflow-hidden p-4 text-center border border-[var(--red)]/40">
                <p className="text-sm font-mono text-[var(--red)]">{copy.title}</p>
                <p className="text-xs text-txt-muted mt-1">{copy.description}</p>
            </div>
        );
    }
                ...
                <div className="flex items-center gap-3">
                    <div className="relative flex-shrink-0">
                        <SafeImage
                            ...
                            fallbackClassName="w-16 h-16 rounded-full bg-white/6 flex items-center justify-center"
                            fallbackLabel={discord_user.username}
                            loading="lazy"
                            decoding="async"
